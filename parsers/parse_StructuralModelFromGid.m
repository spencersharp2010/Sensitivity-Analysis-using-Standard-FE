...
%% 11. If boundary load is applied, then get edge connectivity arrays for the Neumann edges
if strcmp(NBC.loadType(1,:),'boundaryLoad')
    
    disp('>> Boundary load detected');
            ...
            if i ~= j
                ...
                for k = 1:length(indexJ)
                    ...
                    if norm(commonElmnts) ~= 0 && strcmp(NBC.fctHandle(i,:),NBC.fctHandle(j,:))
                        ...
                        % ordering as they are stored in the element array
