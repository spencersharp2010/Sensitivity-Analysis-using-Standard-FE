function [disp_sens, ene_sens]=numerical_adjoint_sensitivity_central(finiteDifference,eneSens, dispSens,analysis,strMsh,homDBC,inhomDBC,NBC,bodyForces,parameters,propStrDynamics,intDomain)
...
%setting up new meshes whose nodal coordinates get updated in each
%iteration
...
for i=1:noNodes
    ...
    %setting up the mesh for current forward perturbation
    ...
    %without body forces
    F_temp_wbf_forward = computeLoadVctFEMPlateInMembraneAction...
        (new_mesh_forward,analysis,NBC,t,intDomain,outMsg);

    [K_temp_forward,F_temp_forward,~] = computeStiffMtxAndLoadVctFEMPlateInMembraneActionCST...
        (analysis,u,uSaved,uDot,uDotSaved,DOFNumbering,new_mesh_forward,F_temp_wbf_forward,loadFactor,...
        bodyForces,propStrDynamics,parameters,intDomain);
    
    %reduction of stiffness matrix and force vector according to boundary
    %conditions
    K_temp_red_forward=K_temp_forward(freeDOFs,freeDOFs);
    F_temp_red_forward=F_temp_forward(freeDOFs);
    
    
    %setting up the mesh for current backward perturbation
    ...
    %computation of current stiffness matrix and force vector
    %without body forces
    F_temp_wbf_backward = computeLoadVctFEMPlateInMembraneAction...
        (new_mesh_backward,analysis,NBC,t,intDomain,outMsg);

    [K_temp_backward,F_temp_backward,~] = computeStiffMtxAndLoadVctFEMPlateInMembraneActionCST...
        (analysis,u,uSaved,uDot,uDotSaved,DOFNumbering,new_mesh_backward,F_temp_wbf_backward,loadFactor,...
        bodyForces,propStrDynamics,parameters,intDomain);
    
    %reduction of stiffness matrix and force vector according to boundary
    %conditions
    K_temp_red_backward=K_temp_backward(freeDOFs,freeDOFs);
    F_temp_red_backward=F_temp_backward(freeDOFs);
    
    %computing the derivative of the stiffness matrix by finite
    %differencing in current configuration
    K_diff_temp = central_finite_difference(K_temp_red_forward,K_temp_red_backward,finiteDifference.perturbation);
    F_diff_temp = central_finite_difference(F_temp_red_forward,F_temp_red_backward,finiteDifference.perturbation);
