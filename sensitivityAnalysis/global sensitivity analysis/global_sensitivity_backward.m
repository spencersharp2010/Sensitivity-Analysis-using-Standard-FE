function [disp_sens, ene_sens,stress_sens]=global_sensitivity_backward(finiteDifference,eneSens,dispSens,vonMisesSens,analysis,strMsh,homDBC,inhomDBC,valuesInhomDBC,NBC,bodyForces,parameters,computeStiffMtxLoadVct,solve_LinearSystem,propNLinearAnalysis,propStrDynamics,intDomain,caseName,pathToOutput,isUnitTest)

%calculates the displacement sensitivity,strain energy sensitivity and von Mises stress sensitivity by
%global backward finite differencing in one go
...
for i=1:noNodes
    ...
    for j=1:n_disp_sens
        ...
        disp_sens(:,1,i,j)= backward_finite_difference(transpose(initial_displacements(j,:)),disp(:,1,i,j),finiteDifference.perturbation);
    ...
    ene_sens(i,1)= backward_finite_difference(initial_strain_energy, strain_energies(i,1),finiteDifference.perturbation);
    end
    ...
    stress_sens(i,1,:)= backward_finite_difference(initial_stresses,s_temp,finiteDifference.perturbation);
    end
...
for i=1:noNodes
    ...
    for j=1:n_disp_sens
        ...
        %sensitivity as the finite difference of the
        %final system results
        disp_sens(:,2,i,j)= backward_finite_difference(transpose(initial_displacements(j,:)),disp(:,2,i,j),finiteDifference.perturbation);
    ...
    %sensitivity as the finite difference of the final
    %strain energy results
    ene_sens(i,2)= backward_finite_difference(initial_strain_energy, strain_energies(i,2),finiteDifference.perturbation);
    end
    ...
    %sensitivity as the finite difference of the final
    %von Mises stress results
    stress_sens(i,2,:)= backward_finite_difference(initial_stresses,s_temp,finiteDifference.perturbation);
    end
