...
%% 2_a. If distributed load is applied, loop over all elements on the Neumann boundary
%%    otherwise, skip to 2_b
if strcmp(NBC.loadType(1,:),'boundaryLoad')
    ...
    for counterEl = 1:length(NBC.lines(:,1))
        ...
        %% 2iv. Assemble the Element Freedom Table (EFT)
...
%% 2_b. If point loads are applied, loop over all applied loads
elseif strcmp(NBC.loadType(1,:),'pointLoad')
    
    number_nodes_with_point_loads = length(NBC.nodes(:,1));
    
    for i = 1:number_nodes_with_point_loads
        nodeID = NBC.nodes(i,1);
        loadFctHandle = str2func(NBC.fctHandle(i,:));
        x1 = strMsh.nodes(nodeID(1),:);
        tractionOnGP = loadFctHandle(x1(1,1),x1(1,2),x1(1,3),t);
        tractionOnGP2D = tractionOnGP(1:2,1);
        F(2*nodeID-1) = tractionOnGP2D(1);
        F(2*nodeID) = tractionOnGP2D(2);
    end
end
%% 3. Appendix
if strcmp(outMsg,'outputEnabled')
    ...
    fprintf('\nComputation of the load vector took %.2d seconds \n\n',computationalTime);
